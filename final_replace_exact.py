...
# Replace _buildCampaignStatusSection down to the start of the next method
# The next method is likely Widget _buildQuickActionsSection() { ? No, wait, look at the last read, what is the next method?
# Let's find the exact end.
start_idx = content.find("  Widget _buildCampaignStatusSection(DashboardState state) {")
if start_idx != -1:
    end_idx = content.find("}\n\nclass _OperationMetric", start_idx)
    if end_idx != -1:
