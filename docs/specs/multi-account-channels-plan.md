...
- **`lib/features/messaging/live_chat/providers/live_chat_provider.dart`** + **`live_chat_screen.dart`**:
  - ~~Mở rộng `loadAccounts()`/`getAccounts()`~~ **[DRIFT — đã sửa lại theo code thực tế]**: `loadAccounts()`/`state.accounts` (nguồn `GET /crm/groups/accounts`) hóa ra **không phải** nguồn dữ liệu của dropdown tài khoản trong `_Header` — dropdown đó (và danh sách account trong `live_chat_settings_dialog.dart`) đọc thẳng `zaloIntegrationProvider.accounts`, bỏ qua `state.accounts` hoàn toàn (kiến trúc có sẵn từ trước, không phải do Giai đoạn E gây ra). Vì vậy điểm mở rộng đúng là **`_Header` widget** trong `live_chat_screen.dart`: watch thêm `workflowAutomationProvider` (đã tự `loadChannelAccounts()` khi provider được khởi tạo) và gộp `state.facebookPages`/`state.tiktokAccounts` vào cùng danh sách item dropdown với `zaloState.accounts`. `LiveChatNotifier.loadAccounts()`/`getAccounts()` giữ nguyên, Zalo-only (không đổi, tránh phá vỡ tính năng Group Management khác đang dùng chung `getAccounts()`).
  - `LiveChatAccount` được thêm field `channel` (mặc định `CrmChannel.zaloPersonal`) để mỗi dòng dropdown hiện đúng icon kênh (tái dùng `CrmChannel.icon`/`.color` từ `lib/shared/models/crm_channel.dart`).
  - Giữ filter-chip theo channel (`_ConversationFilterChip`) làm bộ lọc phụ (lọc thô theo loại nền tảng), dropdown account làm bộ lọc chính (lọc theo tài khoản cụ thể) — không redesign, tái dùng plumbing `selectedAccount`/`accountId` đã hoạt động cho Zalo.
  ...
  - **Bug phát hiện thêm khi test E**: phần gate bật/tắt ô soạn tin (composer) trong conversation detail card đang suy ra `isAccountConnected` từ `zaloState.accounts.firstWhere(... orElse: disconnected_expired)` — với hội thoại Facebook/TikTok, `accountId` không khớp bất kỳ Zalo account nào nên luôn rơi vào nhánh `orElse` → composer bị khóa nhầm. Đã sửa: chỉ áp dụng gate này khi `conversation.channel` là `zaloPersonal`/`zaloOa`; các channel khác mặc định `isAccountConnected = true` (kết nối Facebook/TikTok do integration settings riêng quản lý, không qua Zalo account pool).
...
7. **Giai đoạn G — Instagram** (làm sau khi Giai đoạn A-F multi-account Facebook/TikTok xong): backend enum + webhook route + `instagram-channel.ts` + Flutter enum/settings screen/Live Chat. Verify: giả lập webhook Meta test event có `object:"instagram"`, xác nhận tin vào đúng `LocalChatStore` với `channel:'instagram'` và `accountId` đúng IG-linked Page.
   → **✅ Hoàn thành (2026-07-05)**: `CrmChannelIntegration.js` enum + `channelWebhooks.js` (`GET/POST /api/crm/instagram/webhook`) + `instagram-channel.ts` + đăng ký trong `channel-registry.ts` + `integration-store.ts` (`instagramAccounts[]`) + route account CRUD trong `server.ts` + Flutter (`CrmChannel.instagram`, `workflow_automation_provider.dart`/`_api.dart`, `instagram_settings_screen.dart`, route `/integrations/instagram`, Live Chat account-switcher merge). Verify đã chạy: `tsc --noEmit` (zalo-bot-service) sạch, `flutter analyze` không có lỗi mới, 131 test backend (`npm run test`) pass, 5 test Flutter (`workflow_automation_provider_test.dart`) pass. Doc sync: `PROJECT_SUMMARY.md` và `n8n-facebook-integration-contract.md` đã cập nhật.
8. **Giai đoạn H — WhatsApp**: tương tự Giai đoạn G + banner cảnh báo 24h window. Verify: test webhook WhatsApp Cloud API sandbox, xác nhận gửi trong/ngoài 24h window được cảnh báo đúng trên UI trước khi gửi (nếu quyết định chặn cứng ngoài 24h, cần hỏi lại người dùng — đây là quyết định sản phẩm, không tự quyết).
   → **✅ Hoàn thành (2026-07-06)**: `CrmChannelIntegration.js` enum + `channelWebhooks.js` (`GET/POST /api/crm/whatsapp/webhook`) + `whatsapp-channel.ts` (gửi qua `graph.facebook.com/v19.0/{phone_number_id}/messages`) + đăng ký trong `channel-registry.ts` + `integration-store.ts` (`whatsappAccounts[]`) + route account CRUD trong `server.ts` + Flutter (`CrmChannel.whatsapp`, `workflow_automation_provider.dart`/`_api.dart`, `whatsapp_settings_screen.dart` với checkbox `enforce24hWindow` + tooltip giải thích, route `/integrations/whatsapp`, sidebar). Quyết định sản phẩm: chỉ cảnh báo UI, không chặn cứng ngoài 24h window (giống pattern chưa-enforce hiện có của Facebook/Instagram/TikTok) — người dùng đã xác nhận chọn phương án này. Verify đã chạy: `tsc --noEmit` (zalo-bot-service) sạch, `flutter analyze` không có lỗi mới (chỉ còn warning pre-existing không liên quan), 131 test backend (`npm run test`) pass, `flutter test` không phát sinh regression mới (1 test fail duy nhất là lỗi có sẵn trên `main`, xác nhận qua `git stash` chạy lại không liên quan tới thay đổi này).
9. **Giai đoạn I — Telegram**: chốt Quyết định #1 (field `botToken`) trước, sau đó backend/local bridge/Flutter theo khuôn đơn giản hơn (không app secret). Verify: tạo bot test qua BotFather, gọi `setWebhook` trỏ về route mới, gửi tin thử 2 chiều.
   → **✅ Hoàn thành (2026-07-06)**: Quyết định #1 chốt field `botToken` (không dùng app secret). `CrmChannelIntegration.js` enum + `channelWebhooks.js` (`POST /api/crm/telegram/webhook/:botId`, xác thực qua header `x-telegram-bot-api-secret-token`, không có bước GET verify handshake như kênh Meta) + `telegram-channel.ts` (gửi qua `api.telegram.org/bot{token}/{method}`) + đăng ký trong `channel-registry.ts` + `integration-store.ts` (`telegramBots[]`) + route account CRUD trong `server.ts` (POST tự gọi `getMe` để lấy bot id/username và `setWebhook` với `secret_token` sinh bằng `randomBytes`, DELETE gọi `deleteWebhook`) + Flutter (`CrmChannel.telegram`, `workflow_automation_provider.dart`/`_api.dart`, `telegram_settings_screen.dart` chỉ 2 field tên hiển thị + Bot Token, route `/integrations/telegram`, sidebar). Verify đã chạy: `tsc --noEmit` (zalo-bot-service) sạch, `flutter analyze` không có lỗi mới, 131 test backend (`npm run test`) pass, `flutter test` không phát sinh regression mới.
...
11. **Giai đoạn L — Webchat** (tách riêng, chỉ làm sau khi có xác nhận phạm vi rõ ràng từ người dùng — xem Quyết định #2): thiết kế endpoint public + cơ chế real-time + widget embed, review riêng trước khi code vì đây là bề mặt tấn công mới (public-facing, không qua agent-auth) cần cân nhắc kỹ về bảo mật/rate-limit. → ✅ **Hoàn thành (2026-07-06)**: Backend `utils/webchatEventHub.js` (SSE hub in-process), `routes/webchatPublic.js` (mounted `/api/public/webchat`, CORS mở cho site nhúng bất kỳ; `GET /:widgetId/config`, `GET/POST /:widgetId/messages` rate-limit theo IP+session trả 429 khi lạm dụng, `GET /:widgetId/events` SSE đẩy phản hồi agent real-time), `server/public/webchat/widget.js` (static embed script tại `/webchat/widget.js`), nhánh relay webchat trong `routes/crm.js` dùng chung `upsertConversationFromInbound`. Local bridge: `integration-store.ts` (`webchatWidgets`), `server.ts` (route CRUD widget), `webchat-channel.ts` (adapter, đăng ký qua `POST /agent/channels/register` chung, không OAuth/app-secret vì không có provider ngoài). Flutter: `crm_channel.dart` (`webchat` enum), `workflow_automation_provider.dart`/`_api.dart` (state/API tương tự các kênh khác), `webchat_settings_screen.dart` (danh sách widget + nút "Sao chép mã nhúng"), routing/nav (`app_routes.dart`, `app_router.dart`, `nav_item_models.dart`), và merge `webchatWidgets` vào dropdown chuyển tài khoản Live Chat (`live_chat_screen.dart`). Trong lúc merge dropdown Live Chat phát hiện WhatsApp/Telegram (Giai đoạn H/I) trước đó thực ra chưa từng được nối vào dropdown này dù đã đánh dấu hoàn thành — đã bổ sung luôn cả 2 kênh này cùng lúc (xem `.claude/IMPORTANT_FIXED_BUGS.md`). Verify: `flutter analyze` sạch, `flutter test` chỉ còn 1 lỗi pre-existing không liên quan (`workflow_screen_test.dart`, xác nhận bằng `git stash` trên cây sạch), local bridge `tsc --noEmit` + `npm run bundle` sạch, backend `npm test` xanh toàn bộ sau khi sửa 3 test hợp đồng (`crmLocalFirstContract.test.mjs`, `crmChatbotAgentContract.test.mjs`) đã lỗi thời so với refactor BE-6 (loại bỏ cờ `IS_LOCAL_FIRST_LIVE_CHAT`/`LOCAL_BRIDGE_REQUIRED`, từ 2026-07-03, không liên quan Phase L) — các test này lỗi từ trước, chưa từng được chạy lại nên không ai phát hiện. Chưa thực hiện smoke test thủ công qua trình duyệt thật (môi trường sandbox không có mạng ra ngoài); khuyến nghị người dùng tự kiểm tra luồng gửi tin khách → Live Chat → trả lời → SSE về widget trước khi công bố rộng rãi.
