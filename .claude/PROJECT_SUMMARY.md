...
*Latest Session: Phase 3 complete — TikTok messaging mirrored onto the same cloud-relay architecture as Facebook (Phase 2). Backend: `channelWebhooks.js` gained `GET/POST /api/crm/tiktok/webhook` (verify-token handshake + placeholder `x-tiktok-signature` HMAC check against the per-device `CrmChannelIntegration.appSecret`), reusing the existing `CrmChannelIntegration`/`CrmAgentCommand` model (already generic across channels). `integration/zalo-bot-service`: new `tiktok-channel.ts` (`TiktokChannel implements ZaloChannel`, mirrors `facebook-channel.ts` — placeholder TikTok Business Messaging API base URL, `sendMessage()`/`handleWebhookEvent()`/stub group-account methods) registered in `channel-registry.ts` alongside `FacebookChannel`; `integration-store.ts` gained a `TiktokIntegrationStatus` shape (`accountName`/`accountId`/`appId`/`verifyToken`/`appSecret`/`accessToken`/`enforce24hWindow`) with masking/normalize entries; `server.ts` gained the matching settings-merge + guarded `registerChannelIntegration({channel:'tiktok', ...})` cloud-registration block. `types.ts`'s `ZaloChannelStatus.mode`/`.accountType` literal unions widened to include `'tiktok'`. No changes were needed in `local-chat-api.ts`, `command-executor.ts`, or `agent-runner.ts` — all three were already channel-agnostic from the Phase 1/2 work. Flutter: new `TiktokSettingsState` in `workflow_automation_provider.dart` (parallel to `FacebookSettingsState`) plus `saveTiktokSettings()` on the notifier and API client; new `tiktok_settings_screen.dart` mirrors `facebook_settings_screen.dart`'s structure/hardcoded-Vietnamese-string approach exactly (this project has no vi/en i18n lookup system, unlike the React frontend), with an always-visible disclosure banner flagging the integration as a structural placeholder pending real TikTok Business Messaging API docs/credentials; wired into `app_router.dart`/`app_routes.dart` (`/integrations/tiktok`) and a new sidebar nav item in `nav_item_models.dart`/`app_sidebar.dart`. Every TikTok-specific field name, header name, and endpoint path is an explicitly-commented unverified guess mirroring Facebook's structure, per the plan's Phase 3 instruction. Verified via `flutter analyze` (0 new issues, only a pre-existing unused-parameter warning also present in the Facebook screen it mirrors), `tsc --noEmit` (clean), and `node --check` on the touched backend file. Phase 4 (final documentation pass) complete: `docs/specs/n8n-facebook-integration-contract.md` (renamed to cover TikTok too) updated with the implemented TikTok webhook endpoints and a `tiktok` channel rule, and this file plus `alpha-studio-backend/.claude/PROJECT_SUMMARY.md` brought back in sync with the shipped code. The multi-channel live chat port (Zalo + Facebook Messenger + TikTok) described in the original request is now fully implemented end-to-end, with TikTok explicitly flagged everywhere (code comments, UI banner, docs) as a structural mirror pending verification against real TikTok Business Messaging API credentials.*
...
| `lib/features/workflows/` | Workflow automation feature | Provides `/workflows` (n8n settings, automation rules, template catalog), `/integrations/email` (Email IMAP/SMTP settings), `/integrations/facebook` (Facebook Page Messenger settings). Each has its own screen and sidebar entry under "TÍCH HỢP KÊNH" nav group. |
| `lib/shared/models/crm_channel.dart` | `CrmChannel` enum (shared) | `zaloPersonal`/`zaloOa`/`facebookPage`/`tiktok`/`email`, each with `apiValue`/`label` + `fromApiValue()`. Relocated from `lib/features/workflows/data/workflow_models.dart`, which now `import`s + re-`export`s it so existing call sites are unaffected. Used by `Conversation`/`ChatMessage` (`live_chat_provider.dart`) as the multi-platform Live Chat channel discriminator (defaults to `zaloPersonal`). |
