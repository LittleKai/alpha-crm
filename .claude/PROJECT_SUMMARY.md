...
**Last Updated:** 2026-06-16T19:45:00+07:00

**Session:** #95 - Đóng gói backend Node sidecar thành bundle 1 file thay vì kẹp cả `node_modules`. Thêm `integration/zalo-bot-service/scripts/bundle.mjs` (esbuild bundle `dist/server.js` → `dist/server.cjs`, format CJS, external **chỉ** `better-sqlite3` vì là native addon; banner shim `import.meta.url`→`pathToFileURL(__filename)` để `config.js` tính đúng `projectRoot`, giữ nguyên vị trí `.data/active-port.json`) + script npm `bundle`. `release-to-b2.js` (alpha-studio-backend): bước [3/5] đổi `npm run build`→`npm run bundle`; `stageZaloBackendForWindows` thay copy toàn bộ `node_modules` (98 gói/54MB) bằng `stageDependencyClosure(['better-sqlite3'], ignore:['prebuild-install'])` — chỉ stage closure runtime native (3 gói/~12MB) với `dereference:true` (dứt điểm bug junction #52); launcher `.cmd` ENTRY_JS đổi sang `server.cjs`; `verifyStagedBackend` bỏ check zca-js symlink (nay đã inline trong bundle), thay bằng assert `dist/server.cjs` tồn tại + `better_sqlite3.node` là file thật, smoke-test boot `server.cjs`. Flutter `zalo_backend_manager.dart` nhánh direct-launch ưu tiên `dist/server.cjs`, fallback `server.js` cho dev. Xác minh: `node --check release-to-b2.js` OK; mô phỏng staging prune vào temp dir boot `server.cjs` → `/health` 200, `active-port.json` đúng `<serviceDir>/.data`, không lỗi MODULE_NOT_FOUND; `flutter analyze` file sửa sạch (chỉ còn 1 info doc-comment nền). Lý do KHÔNG dùng "single executable" (SEA/pkg): `better-sqlite3` là native `.node` không nhúng được nên vẫn phải ship file sidecar, cộng fragility ESM→CJS; bundle + externalize native là điểm cân bằng đúng.

**Session:** #94 - Sửa loạt lỗi render Live Chat (theo ảnh `.claude/tmp/img/image.png`). (1) Preview hội thoại hiển thị JSON thô: `_formatConversationPreview` nay bóc tách envelope của bridge `{direction,messageType,content}` (đệ quy một lần) để format đúng text/type/direction thay vì in JSON. (2) Mọi tin nhắn đều dính tag "Tin nhắn đã trả lời": `_mapFromJsonField` trả `null` cho object rỗng `{}` nên `quote` rỗng không còn render hộp trả lời (an toàn cho metadata vì có `?? const {}`). (3) Tên + avatar hội thoại trống (header và avatar bong bóng 1-1): bridge `findOrCreateConversation` không còn ghi đè `displayName`/`avatarUrl` bằng chuỗi rỗng khi tin nhắn outbound (chatbot/operator) tới — dùng `CASE WHEN ? != '' ...`; thêm fallback đọc trong `listConversations`: hội thoại 1-1 thiếu tên/avatar sẽ lấy từ tin nhắn inbound gần nhất có `senderName`. Xác minh: Node build sạch, 91/91; flutter analyze provider sạch; flutter test 102/102 (+2 test: quote rỗng→null, bóc tách envelope preview). Lưu ý cần khởi động lại app để bridge nạp `dist/` mới. (4) Danh sách hội thoại chưa đầy đủ (user chọn hướng "đồng bộ thread Zalo vào bridge"): thêm bootstrap inbox một lần/mỗi tiến trình — khi listener `connected`, gọi `requestOldMessages(ThreadType.User, null)` + `(ThreadType.Group, null)` (WS cmd 510/511, `first:true`, global toàn account) để Zalo trả lịch sử gần đây; handler `old_messages` sẵn có upsert (kèm enrich tên/avatar qua `getOrFetchUserProfile`) → hội thoại populate như messenger. Guard bằng `recentInboxBootstrapDone` Set, reset khi `stopListenerForInstance`. Xác minh logic: Node build sạch + 91/91 (bootstrap cần khởi động lại app + listener connect để verify thực địa số hội thoại tăng).

**Session:** #93 - Sửa lỗi nghiêm trọng: click vào hội thoại Live Chat thì ô tin nhắn trắng hoàn toàn (không load lịch sử chat). Nguyên nhân gốc: tải tin nhắn LUÔN đi qua bridge (`getMessages` dùng `_preferLocalZaloActions || localFirstEnabled` = luôn true), nhưng bản review #92 đã cổng nguồn inbox chỉ theo `localFirstEnabled`. Khi cờ Flutter `localFirstLiveChat` tắt (hoặc link cloud↔bridge gãy), inbox lấy từ cloud → `conversation.id` thành Mongo `_id` của cloud, nhưng bridge có `cloudConversationId=""` nên không resolve được → `getMessages` trả `[]` → ListView tin nhắn không có empty-state nên hiển thị trắng. Khắc phục trong `LiveChatRepository`: (1) cổng nguồn inbox theo CÙNG điều kiện với nguồn tin nhắn `_preferLocalZaloActions || localFirstEnabled` để id-space inbox khớp nguồn tin nhắn (luôn ưu tiên bridge, fallback cloud khi bridge offline); (2) `markRead` ưu tiên bridge khi bridge là nguồn (bridge mark-read chấp nhận cả cloud lẫn local id), chỉ fallback cloud khi bridge offline và không ở chế độ strict local-first. Xác minh thực địa: bridge online, 1 hội thoại `cloudConversationId=""`, 9 tin nhắn resolve đúng theo local UUID. Node 91/91; flutter analyze chỉ còn 2 info `avoid_print` nền; flutter test 100/100.

**Session:** #92 - Live Chat: chuyển nguồn inbox sang local bridge và làm tin cậy toggle chatbot theo từng hội thoại. Backend bridge thêm `ChatbotStore.resolveConversationEnabled()` và đính kèm `chatbotEnabled/chatbotMode/chatbotReason` vào `GET /local/conversations`. Flutter thêm `getLocalConversations()` và định tuyến `LiveChatRepository.getConversations()` qua bridge (fallback cloud khi offline), xử lý sự kiện SSE `conversation.chatbot_state` để bật/tắt công tắc Bot tức thời, thêm guard accountId, và giữ nguyên hội thoại đang mở khi silent reload không chứa nó (sửa lỗi nhảy hội thoại). Sửa khi review: (1) cổng inbox theo `localFirstEnabled` (khớp markRead/updateChatbotState/watchEvents) để tránh lệch id khi cờ tắt; (2) `Conversation.fromJson` ưu tiên `cloudConversationId` làm id để thao tác theo cloud (lưu tags/notes, markRead) vẫn hoạt động khi inbox lấy từ bridge. Hạn chế còn lại: badge tag theo hàng trong inbox local-first chưa hiển thị (bridge không lưu tags); realtime/SSE chỉ chạy khi `localFirstLiveChat=true`. Xác minh: Node 91/91; flutter analyze chỉ còn info nền; flutter test 99/99 (+1 test ưu tiên cloudConversationId, +1 test giữ hội thoại). Chi tiết review tại `.hybrid-ai/REVIEW_LOG.md`.

**Session:** #91 - Sửa lỗi không tìm thấy file thực thi khi chạy ứng dụng Windows (`flutter run -d windows`). Thay đổi thuộc tính `OUTPUT_NAME` của target binary trong `windows/runner/CMakeLists.txt` thành "alpha_crm" để khớp với `BINARY_NAME` "alpha_crm" mà Flutter CLI mong đợi. Cập nhật thuộc tính `OriginalFilename` tương ứng trong `windows/runner/Runner.rc` thành "alpha_crm.exe". Xác minh: `flutter run -d windows` biên dịch và khởi động thành công; `flutter test` vượt qua 99/99 bài kiểm tra; `flutter analyze` sạch lỗi biên dịch.
...
**Session:** #85 - Loại bỏ hoàn toàn danh sách tạm (mock fallback data) trong các UI providers (Customers/Contacts, Message Templates, Send History, và Zalo Group/Friend Automation). Khi truy xuất dữ liệu thất bại hoặc rỗng, trả về danh sách rỗng hoặc lỗi thay vì fallback dữ liệu giả. Giữ lại import các tệp mock định nghĩa kiểu dữ liệu (`Contact`, `MessageTemplate`, `ZaloGroup`, `FriendRecord`, `ScannedMember`, `SavedScannedGroup`) để đảm bảo không lỗi biên dịch. Dọn dẹp các import unused của `package:flutter/foundation.dart` trong `templates_provider.dart` và `customers_provider.dart`. Xác minh: `flutter test` pass 92/92; `flutter analyze` không lỗi biên dịch.

**Session:** #84 - Thay đổi tên hiển thị của ứng dụng thành Alpha CRM ở các nền tảng (Web title/manifest, Windows window title/metadata, Android label) và các vị trí UI (Sidebar branding, Topbar breadcrumbs, Bulk Messaging screen). Cập nhật test suite để kiểm chứng tên hiển thị mới. Viết các prompt mô tả thiết kế logo mới cho Alpha CRM. Xác minh: `flutter test` pass 92/92; `flutter analyze` pass.

**Session:** #83 - Sửa tiếp các lỗi Live Chat Zalo còn tồn tại theo `.claude/prompt.txt`. Backend `personal-zca-channel` không còn dùng fallback sai `cliMsgId = msgId` cho recall/reaction; khi gửi qua `zca-js`, service ép/capture `clientId` Zalo thật và trả `clientMessageId` để SQLite lưu lại, giúp undo/reaction có payload đúng. Local bridge cập nhật `clientMessageId` sau send/retry, trả `data` kèm attachments cho cả `/local/messages/send` và `/local/messages/attachments/send`, đồng thời truyền `cliMsgId` vào reaction. Flutter `LiveChatRepository` ưu tiên bridge cục bộ cho các action Zalo desktop nhưng fallback cloud khi bridge offline ở chế độ không local-first; reaction không còn báo "Live Chat local-first" cho người dùng. UI media bổ sung resolver cho attachment dạng string/local path và fallback theo `messageType`, nên file có tên file và image không rơi về text `[image]`/`[file]` khi biết đó là media. Xác minh: `npm.cmd test` trong `integration/zalo-bot-service` pass 49/49; `flutter test` pass 92/92; focused `flutter test test\live_chat_provider_test.dart test\live_chat_repository_test.dart` pass 15/15 sau tinh chỉnh fallback; `flutter analyze` vẫn exit 1 chỉ vì 68 info lint nền, không có error/warning mới.

**Session:** #82 - Sửa lại 4 lỗi vận hành Live Chat Zalo theo `.claude/prompt.txt`. Flutter `LiveChatNotifier` bắt lỗi reaction/recall và hiển thị `errorMessage` rõ ràng; `LiveChatRepository` không fallback Cloud API khi recall local-first thất bại, tránh UI báo thu hồi giả. Local Zalo bridge giữ nguyên lỗi backend chi tiết, trả message kèm attachments sau `/local/messages/attachments/send`, và SQLite `LocalChatStore` tạo preview sạch (`[Hình ảnh]`, `[Tệp đính kèm]`, tiêu đề link) trước khi cắt ngắn thay vì cắt chuỗi JSON thô. `personal-zca-channel` trích xuất attachment ảnh/file từ payload Zalo (`href/url/thumb/fileName/fileSize/params`) và `agent-runner` truyền attachments vào SQLite cho inbound/realtime/history. Xác minh: `npm.cmd test` pass 48/48; `flutter test` pass 89/89; `flutter analyze` vẫn exit 1 chỉ vì 68 info lint nền, không có error/warning.

**Session:** #81 - Sửa các lỗi Live Chat Zalo và Bulk Messaging sau khi kiểm tra theo `D:\prompt.txt`. Backend `personal-zca-channel` dùng chung `createZaloClient()` có `imageMetadataGetter`, đọc kích thước PNG/JPEG/GIF/WebP cho gửi ảnh, map reaction UI sang zca-js, chuẩn hóa quote reply, và gọi đúng `api.undo`/fallback undo cho recall. Flutter Live Chat không còn lọc mất nhóm khi danh sách managed rỗng, hiển thị ảnh/file local vừa gửi, và preview hội thoại không leak JSON attachment. Bulk Messaging giữ/clear tài khoản đúng khi danh sách account reload, không truyền `all_accounts` xuống campaign, tab nhóm/bạn bè clear filter để tải toàn bộ khi chọn "Toàn bộ tài khoản", nút tải nhóm gọi sync thật, và preview nội dung hỗ trợ `{{tên}}`, `{{sdt}}`, `{{nhóm}}`, markdown Unicode, spintax deterministic. Xác minh: `npm.cmd run build` cho `integration/zalo-bot-service` pass; `node --test dist/channels/personal-zca-channel.test.js` pass 7/7; focused Flutter tests `zalo_text_formatter_test`, `bulk_messaging_provider_test`, `live_chat_provider_test` pass 15/15; `flutter analyze` không có error/warning mới nhưng vẫn exit 1 vì 69 info lint nền.
