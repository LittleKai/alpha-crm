...
*Latest Session: Fixed five production stability defects — Live Chat now always opens the local realtime SSE stream on desktop (`watchEvents()` gate fixed to `_preferLocalZaloActions || localFirstEnabled`, was `localFirstEnabled`-only) with a 60s SSE inactivity timeout so a dead socket surfaces as an error instead of hanging silently; the desktop backend supervisor (`zalo_backend_manager.dart`) raised its health-probe timeout 2s→5s and failure threshold 2→3 misses to tolerate synchronous `better-sqlite3` queries, added a tick re-entrancy guard, made the circuit breaker auto-retry once per 5-minute cooldown instead of latching forever, and fixed a stale exit-listener race in `waitUntilReady`; the backend now also traps `uncaughtException` (in addition to `unhandledRejection`) so a synchronous throw inside a zca-js callback no longer crashes the process.*
